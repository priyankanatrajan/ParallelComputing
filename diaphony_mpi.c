...
*/
{
  ...
  MPI_Status status;
  int my_id, parent, ierr, o, num_rows, num_procs,
         an_id, num_rows_to_receive, rows_avg, 
         sender, num_rows_received, start_row, end_row, num_rows_to_send;
  int rows;
  rows = file_row_count("input");  
  double points2[rows];
  r8mat_header_read ( "input", &dim_num, &point_num );
  points = r8mat_data_read ( "input", dim_num, point_num );
  //Initialize MPI
  ...
  if(my_id == 0)
	{
			rows_avg = rows/num_procs;
			for(an_id = 1; an_id < num_procs; an_id++) 
      {
          start_row = an_id*rows_avg + 1;
          end_row   = (an_id + 1)*rows_avg;

          num_rows_to_send = end_row - start_row + 1;

          ierr = MPI_Send( &num_rows_to_send, 1 , MPI_DOUBLE,
                  an_id, send_data_tag, MPI_COMM_WORLD);

          ierr = MPI_Send(&points[start_row], num_rows_to_send, MPI_DOUBLE,
                  an_id, send_data_tag, MPI_COMM_WORLD);
      }
  		for(i = 0; i < rows_avg + 1; i++) 
      {
          for ( j = 0; j < rows_avg + 1; j++ )
  				{
  		   			prod = 1.0;
  						for ( k = 0; k < dim_num; k++ )
  						{
      ...
      for(an_id = 1; an_id < num_procs; an_id++)
      {
          ...
          ierr = MPI_Recv( &partial_d, 1, MPI_LONG, MPI_ANY_SOURCE,return_data_tag, MPI_COMM_WORLD, &status);
    ...
    {
        ierr = MPI_Recv( &num_rows_to_receive, 1, MPI_DOUBLE, 
           parent, send_data_tag, MPI_COMM_WORLD, &status);
            
        ierr = MPI_Recv( &points2, num_rows_to_receive, MPI_DOUBLE, 
                 parent, send_data_tag, MPI_COMM_WORLD, &status);
        ...
        for(i = 0; i < num_rows_received; i++)
        {
            for ( j = 0; j < num_rows_received; j++ )
            {
                ...
                partial_d = partial_d + prod - 1.0;
            } 
        }
        ierr = MPI_Send( &partial_d, 1, MPI_LONG, parent, 
